...
uint8_t const *tud_descriptor_device_cb(void)
{
  ...
  if (!logged)
  {
    auto *desc = reinterpret_cast<tusb_desc_device_t *>(descriptor_buffer);
    printf("usb: device desc vid=%04x pid=%04x\r\n", desc->idVendor, desc->idProduct);
    logged = true;
  }
...
uint8_t const *tud_descriptor_configuration_cb(uint8_t index)
{
  ...
  if (!logged)
  {
    printf("usb: config desc interfaces=%u total_len=%u\r\n",
           static_cast<unsigned>(interfaces), config->wTotalLength);
    logged = true;
  }
  return descriptor_buffer;
}
...
char const *string_desc_arr[STRID_COUNT] =
    ...
    "Xbox 360 Controller for Windows", // 2: Product
    serial};                    // 3: Serials will use unique ID if possible

static const OS_DESCRIPTOR microsoft_os_desc = {
...
{
  (void)langid;
  static bool logged_serial = false;
  static bool logged_msft = false;
  // printf("desc %02x %02x\r\n", index, langid);
  ...
  case 0xEE:
    ...
    if (!logged_msft)
    {
      printf("usb: host requested MS OS descriptor 0xEE\r\n");
      logged_msft = true;
    }
    return reinterpret_cast<uint16_t const *>(&microsoft_os_desc);

  case STRID_SERIAL:
    pico_get_unique_board_id_string(serial, sizeof(serial));
    if (!logged_serial)
    {
      printf("usb: host requested serial descriptor\r\n");
      logged_serial = true;
    }
