...
namespace
{
    constexpr uint16_t XBOX_360_CONTROLLER_VID = 0x045e;
    constexpr uint16_t XBOX_360_CONTROLLER_PID = 0x028e;
}
                ...
                else if (request->wValue == INPUT_CAPABILITIES_WVALUE)
                {
                    if (stage == CONTROL_STAGE_SETUP)
                    {
                        ...
                        XInputInputCapabilities_t XInputInputCapabilities = {
                            ...
                            leftTrigger : 0xff,
                            rightTrigger : 0xff,
                            leftThumbX : XBOX_360_CONTROLLER_VID,
                            leftThumbY : XBOX_360_CONTROLLER_PID,
...
void XInputGamepadDevice::device_descriptor(tusb_desc_device_t *desc)
{
    ...
    desc->idVendor = XBOX_360_CONTROLLER_VID;
    desc->idProduct = XBOX_360_CONTROLLER_PID;
    if (!logged)
    {
        printf("xinput: gamepad descriptor subtype=%d vid=%04x pid=%04x\r\n",
               subtype, desc->idVendor, desc->idProduct);
        logged = true;
    }
}
...
void XInputSecurityDevice::device_descriptor(tusb_desc_device_t *desc)
{
    ...
    if (!logged)
    {
        printf("xinput: security descriptor vid=%04x pid=%04x\r\n",
    ...
    if (request->bmRequestType_bit.direction == TUSB_DIR_IN)
    {
        ...
        case 0x81:
            if (stage == CONTROL_STAGE_SETUP)
            {
                ...
                xsm3_set_vid_pid(serial, XBOX_360_CONTROLLER_VID, XBOX_360_CONTROLLER_PID);
