...
void GamepadButtonMapping::update(bool full_poll, bool send_events)
{
    const bool previous_value = m_last_value;
    ButtonMapping::update(full_poll, send_events);
    if (m_last_value == previous_value)
    {
        return;
    }
    ...
    printf("Button %c %s\n", button, m_last_value ? "pressed" : "released");
}
