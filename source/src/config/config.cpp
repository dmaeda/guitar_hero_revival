...
namespace
{
    constexpr uint32_t DEFAULT_EMPTY_PROFILE_ID = 1;
    constexpr SubType DEFAULT_EMPTY_PROFILE_SUBTYPE = Gamepad;
    constexpr PinMode DEFAULT_EMPTY_PROFILE_PIN_MODE = PullUp;

    struct DefaultGamepadButtonBinding
    {
        uint8_t pin;
        GamepadButtonType button;
    };

    proto_Input make_default_gpio_input(uint8_t pin)
    {
        proto_Input input = proto_Input_init_zero;
        input.which_input = proto_Input_gpio_tag;
        input.input.gpio.pin = pin;
        input.input.gpio.pinMode = DEFAULT_EMPTY_PROFILE_PIN_MODE;
        input.input.gpio.analog = false;
        return input;
    }

    void add_default_gamepad_button_mapping(const std::shared_ptr<Profile> &profile,
                                            uint8_t pin,
                                            GamepadButtonType button)
    {
        proto_Mapping proto_mapping = proto_Mapping_init_zero;
        proto_mapping.mapping.which_mapping = proto_Output_gamepadButton_tag;
        proto_mapping.mapping.mapping.gamepadButton = button;
        proto_mapping.input = make_default_gpio_input(pin);

        auto input = InputFactory::create_input(proto_mapping.input, profile);
        auto mapping = MappingFactory::create_mapping(proto_mapping, profile, std::move(input), profile->mappings.size());
        if (mapping)
        {
    ...
    void add_default_empty_mappings(const std::shared_ptr<Profile> &profile)
    {
        static constexpr DefaultGamepadButtonBinding bindings[] = {
            {2, Gamepad_DpadUp},
            {3, Gamepad_DpadDown},
            // GP4/GP5 are reserved for the UART1 debug console, so DpadLeft/DpadRight moved here.
            {13, Gamepad_DpadLeft},
            {14, Gamepad_DpadRight},
            {6, Gamepad_Back},
            {7, Gamepad_Start},
            {8, Gamepad_Guide},
            {9, Gamepad_A},
        ...
        {
            add_default_gamepad_button_mapping(profile, binding.pin, binding.button);
        }
    }

    std::shared_ptr<Profile> make_default_empty_profile()
    {
        auto profile = std::make_shared<Profile>();
        strncpy(profile->name, "Xbox 360 Controller", sizeof(profile->name) - 1);
        profile->subtype = DEFAULT_EMPTY_PROFILE_SUBTYPE;
        profile->mode = ModeXbox360;
        profile->profile_id = DEFAULT_EMPTY_PROFILE_ID;
        add_default_empty_mappings(profile);
        return profile;
    }
}
...
bool load_empty()
{
    ...
    printf("load_empty: initializing default Xbox 360 gamepad profile\r\n");

    config_mgr.set_current_mode(ModeXbox360);
    config_mgr.request_mode(ModeXbox360);
    printf("load_empty: mode set to %d\r\n", ModeXbox360);
    ...
    printf("load_empty: xinput security interface=%u\r\n", secDevice->interface_id);

    auto default_profile = make_default_empty_profile();
    profile_mgr.add_profile(default_profile->profile_id, default_profile);
    printf("load_empty: added profile id=%lu subtype=%d mappings=%u\r\n",
        static_cast<unsigned long>(default_profile->profile_id), default_profile->subtype,
        static_cast<unsigned>(default_profile->mappings.size()));
    profile_mgr.assign_profile_to_devices(default_profile, ProfileAssignMask_AssignUsb,
                                          ModeXbox360, EmulationDeviceConfig{});
    printf("load_empty: usb instances after default profile=%u\r\n",
        static_cast<unsigned>(profile_mgr.usb_instance_count()));
    printf("load_empty: total usb instances=%u\r\n",
           static_cast<unsigned>(profile_mgr.usb_instance_count()));
    return true;
}
